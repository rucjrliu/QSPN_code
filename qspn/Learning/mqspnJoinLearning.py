from Structure.mqspnJoinBase import MultiQSPN, Table_Models_Partitions, Partition, loc2no
...
def partition_table_by_joinkeys(name: str, data_table: pd.DataFrame, JOINKEY_PARTITION_WIDTH_LIMIT, global_joinkeys_cuts: list, th_joinkeys: list):
    #print(name)
    #print(data_table)
    ...
    qspns = np.full(partitions_shape, None)
    ...
    assert check_n == data_table.shape[0], "table {} after partition_table_by_joinkeys, check_n = {} while should be {}".format(name, check_n, data_table.shape[0])
    #print('check_n',check_n)
    #print()
    #return None, None
    return Table_Models_Partitions(name, partitions_loc_no, partitions, qspns), partitions_data

def calc_partition(partition_data: np.ndarray, th_joinkeys: list):
...
def train_partition_qspn(partition_data: pd.DataFrame, filtering_columns: dict, workload_filtering: np.ndarray, queries, rdc_sample_size, rdc_strong_connection_threshold, multivariate_leaf, threshold, wkld_attr_threshold, wkld_attr_bound, qdcorr):
    ...
    parametric_types = [Categorical for i in range(len(filtering_columns))]
...
def jsum_partition_tables(mqspn: MultiQSPN, data_tables: dict, table_th_joinkeys: dict):
    ...
    for name, data_table in data_tables.items():
        ...
        # print()
        mqspn.table_models_partitions[name], table_partitions_data[name] = partition_table_by_joinkeys(name, data_table, mqspn.JOINKEY_PARTITION_WIDTH_LIMIT, global_joinkeys_cuts, table_th_joinkeys[name])
...
def jproduct_join_filtering(models_partitions: Table_Models_Partitions, partitions_data: np.ndarray, th_joinkeys: list, filtering_columns: dict, workload_filtering: np.ndarray, queries, rdc_sample_size, rdc_strong_connection_threshold, multivariate_leaf, threshold, wkld_attr_threshold, wkld_attr_bound, qdcorr):
    for no, partition_data in np.ndenumerate(partitions_data):
        if partition_data is not None:
            models_partitions.partitions[no] = calc_partition(partition_data, th_joinkeys)
            # continue
            if len(filtering_columns) > 0:
                models_partitions.qspns[no] = train_partition_qspn(partition_data, filtering_columns, workload_filtering, queries, rdc_sample_size, rdc_strong_connection_threshold, multivariate_leaf, threshold, wkld_attr_threshold, wkld_attr_bound, qdcorr)
                assert models_partitions.qspns[no] is not None, 'patition{} on data.shape={} qspn training FAILED!'.format(no, partition_data.shape)
...
def learn_multi_QSPN(
    ...
    #     print()
    # exit(-1)
    table_partitions_data = jsum_partition_tables(mqspn, data_tables, table_th_joinkeys)
    #exit(-1)
    ...
    for t, partitions_data in table_partitions_data.items():
        print(t)
        jproduct_join_filtering(mqspn.table_models_partitions[t], partitions_data, table_th_joinkeys[t], mqspn.table_filtering_columns[t], table_workloads[t], table_workloads_patterns[t], rdc_sample_size, rdc_strong_connection_threshold, multivariate_leaf, threshold, wkld_attr_threshold, wkld_attr_bound, qdcorr)
    # print()
