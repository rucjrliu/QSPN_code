...
def loadmodel_MultiQSPN_table_models_partitions_qspns(mqspn: MultiQSPN):
    ...
    for i in mqspn.table_models_partitions.values():
        ...
        for jth, j in np.ndenumerate(i.partitions):
            if j is not None:
                if j.qspn is not None:
                    model = FSPN()
                    model.model = j.qspn
                    model.store_factorize_as_dict()
                    j.qspn = model
                if j.speval_qspn is not None:
                    speval_model = FSPN()
                    speval_model.model = j.speval_qspn
                    speval_model.store_factorize_as_dict()
                    j.speval_qspn = speval_model
    #mqspn._print(info_level=2)
    return mqspn